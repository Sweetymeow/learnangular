...
function makeViz() {
    ...
    d3.json("../data/network_ReturnNetworkData_V2_Orbit_flat.json", function (data) {
        drawOrbit(data)
    });
...
function drawOrbit(_data) {
    ...
    var instTypeList = d3.map(_data.children, function (d) {
        return d["Institution Type"];
    });
    ...
    if (dataTree.length >= 1) {
        dataTree.forEach(function (item) {
            orbitMode.push(item.values.length);
        });
    }
    ...
    orbit = d3.layout.orbit().size([orbitRadius, orbitRadius])
        ...
        .speed(.1)
        .mode(orbitMode.length ? orbitMode : "flat")
        //        .mode([35, 36, 8, 3, 1])
        .nodes(_data);
